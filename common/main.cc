...
void print_help()
{
  ...
  cout << "tmux-mem-cpu-load v" << tmux_mem_cpu_load_VERSION << endl
    ...
    << "\tSet tmux status refresh interval in seconds. Default: 0.5 second\n"
...
{
  unsigned cpu_usage_delay = 990000/2;
  ...
  while( (c = getopt_long( argc, argv, "hi:cpqvl:r:g:m:a:t:", long_options, NULL) ) != -1 )
  {
    switch( c )
    {
      ...
      case 'i': // --interval, -i
        if( atoi( optarg ) < 0.5 )
          {
            std::cerr << "Status interval argument must be 0.5 or greater.\n";
            return EXIT_FAILURE;
          }
